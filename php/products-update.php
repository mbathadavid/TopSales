...
if(isset($_POST['productowner']) || isset($_POST['productname']) || isset($_POST['Statusselect']) || isset($_POST['productscategory']) || isset($_POST['brand']) || isset($_POST['price']) || isset($_POST['proddescription']) || isset($_POST['files'])){
	...
	for ($i=0; $i <$imageCount ; $i++) { 
		...
		$targetpath = "D:/xammpstore/htdocs/Class/products/".$imageName;
		if (move_uploaded_file($imageTempName, $targetpath)) {
		$proupload = $shop->updateproduct($sn,$productowner,$productname,$productscategory,$brand,$Price,$Status,$proddescription,$picture);
		 if ($proupload === true) {
		 	echo "success";
		 }
		 else{
